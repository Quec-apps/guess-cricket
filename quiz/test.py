_str = '''Who holds the record for the highest individual score in a One Day International (ODI) cricket match?
A) Sachin Tendulkar
B) Virender Sehwag
C) Martin Guptill
D) Rohit Sharma
...
for line in _str.splitlines():
    ...
    if cou == 6:
        index_ = line[8]
        ans = returnNum(index_)
        newVar+=f'window["ans"+f] = `{ansList[ans-1]}`\n'
        ansList.pop(ans-1)
        count=1
        for i in ansList:
            newVar+=f'window["noans{count}"+f] = `{i}`;\n'
            count+=1
        ansList = []
    if cou == 7:
        cou=0
